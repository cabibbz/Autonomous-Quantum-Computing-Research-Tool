"""
Sprint 039b: Fill n=12 Potts MI-CV gaps (g=0.9, 1.05, 1.1).
These points are needed to match n=16 for collapse analysis.
"""
...
print("=== Sprint 039b: Potts n=12 fill ===\n")
sys.stdout.flush()

# Load existing n=12 data
with open('results/sprint_038b2_potts_fast.json') as f:
    prior = json.load(f)

n12_data = dict(prior['sizes']['12'])
...
for g_J in g_fill:
    ...
    sys.stdout.flush()
    n12_data[str(g_J)] = {'cv': float(cv), 'time': float(dt)}

results = {
    'experiment': '039b',
    'model': 'q=3 Potts (clock)',
    'n12_complete': n12_data,
    'total_runtime': time.time() - t0,
}
with open('results/sprint_039b_potts_n12_fill.json', 'w') as f:
    json.dump(results, f, indent=2)

print(f"\nTotal runtime: {time.time()-t0:.1f}s")
print(f"n=12 total points: {len(n12_data)}")
