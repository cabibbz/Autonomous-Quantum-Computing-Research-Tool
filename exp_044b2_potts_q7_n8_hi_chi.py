#!/usr/bin/env python3
"""Sprint 044b2: q=7 Potts MI-CV sweep at n=8 with higher chi.
Previous run failed at g>=0.35 with chi=10. Use chi=20 and relax trunc_err.
"""
import numpy as np
...
# ── n=8 sweep with higher chi ──
q = 7
n = 8
# Focus on transition region + extend to disordered phase
g_values = [0.20, 0.25, 0.30, 0.35, 0.40, 0.50, 0.60]

# Load previous results for g<=0.30
prev = [
    {'g': 0.15, 'cv': 0.6122, 'E0': -7.227807, 'status': 'ok'},
]

results_all = []
print(f"=== q={q} Potts n={n} MI-CV sweep (chi=20) ===")
t_start = time.time()
...
for g in g_values:
    ...
    try:
        ...
        dt = time.time() - t0
        print(f"  g={g:.2f}: CV={cv:.4f}, E0={E0:.6f}, DMRG={t_dmrg:.1f}s, MI={t_mi:.1f}s")
        results_all.append({
    ...
    # Save incrementally
    with open('results/sprint_044b_q7_n8_sweep.json', 'w') as f:
        json.dump({'sprint': '044b', 'q': q, 'n': n, 'chi_max': 20,
...
print("\n=== CV PROFILE ===")
# Include g=0.15 from previous run
print(f"  g=0.15: CV=0.6122 (from chi=10)")
for r in ok:
    bar = '█' * int(r['cv'] * 20)
