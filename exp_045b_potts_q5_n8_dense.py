#!/usr/bin/env python3
"""Sprint 045b: Dense q=5 Potts MI-CV sweep at n=8, chi=20 near g_c≈0.41.

Need consistent chi=20 data at n=8 for collapse with n=12 data.
g values: 0.30, 0.35, 0.38, 0.40, 0.42, 0.45, 0.50, 0.55
"""
import numpy as np
...
n = 8
chi = 20
g_values = [0.30, 0.35, 0.38, 0.40, 0.42, 0.45, 0.50, 0.55]

results_all = []
print(f"=== q={q} Potts n={n} chi={chi} dense sweep ===")
t_start = time.time()
...
for g in g_values:
    ...
    with open('results/sprint_045b_q5_n8_dense.json', 'w') as f:
        json.dump({'sprint': '045b', 'q': q, 'n': n, 'chi_max': chi,
                   'data': results_all, 'total_time': time.time()-t_start}, f, indent=2)

t_total = time.time() - t_start
print(f"\nTotal: {t_total:.1f}s")
print("\nResults saved to results/sprint_045b_q5_n8_dense.json")
