#!/usr/bin/env python3
"""Sprint 054a: Central charge c(q=3) at true g_c=1/3.

Extract c from entropy scaling S(n/2) = (c/6)ln(n) + const.
CFT prediction: c = 4/5 = 0.800.
Use DMRG (PottsChain) at g_c=1/3 for n=8,12,16,24,32,48.
"""
...
for n in sizes:
    if time.time() - total_start > 240:
        ...
        break
    print(f"\n=== q={q}, n={n}, g_c={g_c:.6f}, chi_max={chi_max} ===", flush=True)
    E, S, chi, dt = run_dmrg(q, n, g_c, chi_max)
    print(f"  E={E:.8f}, S_half={S:.6f}, chi={chi}, t={dt:.1f}s", flush=True)
    results.append({'n': n, 'E': float(E), 'S_half': float(S), 'chi_actual': chi, 'time': dt})

    # Save incrementally
    with open('results/sprint_054a_central_charge_q3.json', 'w') as f:
        json.dump({'sprint': '054a', 'q': q, 'g_c': g_c, 'chi_max': chi_max,
...
if len(results) >= 3:
    ...
    with open('results/sprint_054a_central_charge_q3.json', 'w') as f:
        json.dump({'sprint': '054a', 'q': q, 'g_c': g_c, 'chi_max': chi_max,
                   'data': results, 'analysis': results_summary}, f, indent=2)
