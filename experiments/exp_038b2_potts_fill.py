"""
Sprint 038b2: Complete Potts MI-CV data for n=8 and add n=12.
"""
...
op_names_all = ['Id'] + op_names_gm

def entropy(rho):
    ev = la.eigvalsh(rho); ev = ev[ev > 1e-15]
    return float(-np.sum(ev * np.log2(ev)))

def mi_cv_potts(n, g_J, chi_max=60):
    ...
    corr = {(a, b): psi.correlation_function(a, b) for a in op_names_gm for b in op_names_gm}
...
print("=== Sprint 038b2: Potts MI-CV (fill) ===\n")
...
# Complete n=8 (missing: 1.1, 1.2, 1.3, 1.5, 2.0)
missing_8 = [1.1, 1.2, 1.3, 1.5, 2.0]
print("--- Completing n=8 ---")
for g_J in missing_8:
    ...
    if elapsed > 25:
        ...
        break
    t1 = time.time()
    cv = mi_cv_potts(8, g_J, chi_max=40)
    dt = time.time() - t1
    print(f"  g/J={g_J:.2f}: CV={cv:.4f} ({dt:.1f}s)")
    results['sizes']['8'][str(g_J)] = {'cv': float(cv), 'time': float(dt)}

# n=12 with fewer points
print("\n--- n=12 (sparse sweep) ---")
g_vals_12 = [0.5, 0.8, 0.9, 1.0, 1.1, 1.3]
results['sizes']['12'] = {}
for g_J in g_vals_12:
    ...
    t1 = time.time()
    try:
        cv = mi_cv_potts(12, g_J, chi_max=60)
        dt = time.time() - t1
        print(f"  g/J={g_J:.2f}: CV={cv:.4f} ({dt:.1f}s)")
...
with open('results/sprint_038b2_potts_fill.json', 'w') as f:
