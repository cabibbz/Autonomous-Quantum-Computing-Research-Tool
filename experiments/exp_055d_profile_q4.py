#!/usr/bin/env python3
"""Sprint 055d: Central charge from entropy profile — Potts q=4 at g_c=0.392.
CFT prediction: c = 1 (Ashkin-Teller, marginal with log corrections).
Sprint 054 found flat overshoot c≈1.23. Does profile method do better?
"""
...
data = []
for n in [16, 24, 32]:
    E0, S_prof, chi_act, dt = run_and_extract(4, n, 0.392, 20)
    ...
    results = {'experiment': '055d', 'model': 'Potts', 'q': 4, 'g_c': 0.392,
               'c_cft': 1.000, 'data': data}
    with open('results/exp_055d.json', 'w') as f:
        json.dump(results, f, indent=2)

print(f"\nc converges: {' -> '.join(f'{d['c_profile']:.4f}' for d in data)}")
print(f"CFT prediction: 1.000")
print("Saved results/exp_055d.json")
