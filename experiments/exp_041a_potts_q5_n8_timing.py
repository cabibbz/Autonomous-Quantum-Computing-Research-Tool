"""
Sprint 041a: q=5 clock MI-CV at n=8 — timing test.
d=5 means 24 SU(5) generators, 576 correlation pairs per site pair.
Need to verify this fits in 60s budget for a few g values.
"""
...
# Generalized Gell-Mann matrices for SU(5): d^2-1 = 24 generators
gm_mats = []
...
print(f"Number of SU({d}) generators: {len(gm_mats)}")
...
def mi_cv_q5(n, g_J, chi_max=15):
    model = ClockChain({'L': n, 'q': 5, 'J': 1.0, 'g': g_J,
    ...
    if len(mi_pos) < 2: return 0.0, t_corr_end - t_corr
    return float(np.std(mi_pos) / np.mean(mi_pos)), t_corr_end - t_corr

print("=== Sprint 041a: q=5 Clock MI-CV at n=8 — Timing Test ===\n")
sys.stdout.flush()

results = {'experiment': '041a', 'model': 'q=5 clock', 'n': 8, 'chi_max': 15,
           'n_generators': len(gm_mats), 'data': {}}

# Just 2 test points: deep ordered and near transition
g_vals = [0.5, 1.0]
...
for g_J in g_vals:
    ...
    t1 = time.time()
    try:
        cv, t_corr = mi_cv_q5(8, g_J, chi_max=15)
        dt = time.time() - t1
        print(f"  g/J={g_J:.2f}: CV={cv:.4f} ({dt:.1f}s, corr={t_corr:.1f}s)")
        results['data'][str(g_J)] = {'cv': float(cv), 'time': float(dt), 'corr_time': float(t_corr)}
    except Exception as e:
        print(f"  g/J={g_J:.2f}: FAILED: {e}")
        results['data'][str(g_J)] = {'cv': None, 'error': str(e)}
    sys.stdout.flush()
    results['total_runtime'] = time.time() - t0
    with open('results/sprint_041a_q5_n8_timing.json', 'w') as f:
