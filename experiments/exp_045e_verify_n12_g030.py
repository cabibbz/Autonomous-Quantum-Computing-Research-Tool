#!/usr/bin/env python3
"""Sprint 045e: Verify n=12 g=0.30 (Sprint 042d value 0.134 used Gell-Mann method).
Also get g=0.50, 0.55 at n=16 for more collapse data.
"""
import numpy as np
...
# Verify n=12 g=0.30
print("=== Verify n=12 g=0.30 (042d: 0.1342 via Gell-Mann) ===")
cv, E0, mi_vals, t_dmrg, t_mi = mi_cv(12, q, 0.30, chi)
print(f"  Direct MPS: CV={cv:.4f} (vs 042d: 0.1342)")
results['n12_g030'] = {'cv': cv, 'E0': E0, 'mi_mean': float(np.mean(mi_vals))}

# Save
with open('results/sprint_045e_verify.json', 'w') as f:
    json.dump({'sprint': '045e', 'q': q, 'chi': chi, 'results': results}, f, indent=2)

# n=16 additional points
print("\n=== n=16 additional points ===")
for g in [0.38, 0.42, 0.50]:
    cv, E0, mi_vals, t_dmrg, t_mi = mi_cv(16, q, g, chi)
    print(f"  n=16 g={g:.2f}: CV={cv:.4f}, DMRG={t_dmrg:.1f}s")
    results[f'n16_g{g:.2f}'] = {'cv': cv, 'E0': E0, 'mi_mean': float(np.mean(mi_vals))}
    with open('results/sprint_045e_verify.json', 'w') as f:
        json.dump({'sprint': '045e', 'q': q, 'chi': chi, 'results': results}, f, indent=2)

# Full consolidated data
print("\n=== CONSOLIDATED q=5 Potts MI-CV (all direct MPS, chi=20) ===")
n8 = {0.30: 0.3732, 0.35: 0.3554, 0.38: 0.3141, 0.40: 0.2755, 0.42: 0.2325,
      0.45: 0.1700, 0.50: 0.1072, 0.55: 0.1324}
n12 = {0.35: 0.3671, 0.38: 0.3399, 0.40: 0.2993, 0.42: 0.2456, 0.45: 0.1677, 0.50: 0.1212}
n12[0.30] = cv  # from verification above
n16 = {0.30: 0.4818, 0.35: 0.3559, 0.40: 0.3100, 0.45: 0.1604}
for key, r in results.items():
    if key.startswith('n16_'):
        g = float(key.split('_g')[1])
        n16[g] = r['cv']

for label, data in [('n=8', n8), ('n=12', n12), ('n=16', n16)]:
    vals = [f"g={g:.2f}:{v:.4f}" for g, v in sorted(data.items())]
    print(f"  {label}: {', '.join(vals)}")

# Crossing analysis
print("\n=== CROSSING n8-n12 ===")
for g in sorted(set(n8.keys()) & set(n12.keys())):
    d = n12[g] - n8[g]
    print(f"  g={g:.2f}: n8={n8[g]:.4f}, n12={n12[g]:.4f}, delta={d:+.4f}")

print("\n=== CROSSING n12-n16 ===")
for g in sorted(set(n12.keys()) & set(n16.keys())):
    d = n16[g] - n12[g]
    print(f"  g={g:.2f}: n12={n12[g]:.4f}, n16={n16[g]:.4f}, delta={d:+.4f}")

print("\n=== CROSSING n8-n16 ===")
for g in sorted(set(n8.keys()) & set(n16.keys())):
    d = n16[g] - n8[g]
    print(f"  g={g:.2f}: n8={n8[g]:.4f}, n16={n16[g]:.4f}, delta={d:+.4f}")

with open('results/sprint_045e_verify.json', 'w') as f:
    json.dump({'sprint': '045e', 'q': q, 'chi': chi, 'results': results,
               'consolidated': {'n8': {str(k):v for k,v in n8.items()},
                                'n12': {str(k):v for k,v in n12.items()},
                                'n16': {str(k):v for k,v in n16.items()}}}, f, indent=2)
print("\nSaved to results/sprint_045e_verify.json")
