#!/usr/bin/env python3
"""Sprint 054c: Central charge c(q=5) at true g_c=0.441.

Extract c from entropy scaling S(n/2) = (c/6)ln(n) + const.
NO CFT prediction — 2D classical q=5 Potts is first-order.
But 1D quantum is second-order (Sprint 042-043). c(q=5) is POTENTIALLY NOVEL.
"""
...
q = 5
g_c = 0.441  # from energy gap Sprint 051
results = []
...
for n in sizes:
    ...
    with open('results/sprint_054c_central_charge_q5.json', 'w') as f:
        json.dump({'sprint': '054c', 'q': q, 'g_c': g_c, 'chi_max': chi_max,
...
if len(results) >= 3:
    ...
    print(f"Full fit: c = {c_full:.4f} (no CFT prediction for q=5)")
    ...
    with open('results/sprint_054c_central_charge_q5.json', 'w') as f:
        json.dump({'sprint': '054c', 'q': q, 'g_c': g_c, 'chi_max': chi_max,
                   'data': results, 'analysis': {
                       ...
                       'c_prediction': 'NONE - 2D classical is first-order'}}, f, indent=2)
