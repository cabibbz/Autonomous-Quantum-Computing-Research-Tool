#!/usr/bin/env python3
"""Sprint 047b: q=7 Potts MI-CV sweep at n=12, chi=20.

Direct MPS contraction for all-pairs MI. Focused sweep around g_c≈0.259.
Fewer points than n=8 due to longer DMRG time.
"""
import numpy as np
...
n = 12
chi = 20
# Focused sweep: key points around g_c=0.259
g_values = [0.15, 0.20, 0.24, 0.26, 0.28, 0.30, 0.35, 0.40]

results_all = []
print(f"=== q={q} Potts n={n} chi={chi} focused sweep (direct MPS) ===")
t_start = time.time()
...
for g in g_values:
    ...
    with open('results/sprint_047b_q7_n12.json', 'w') as f:
        json.dump({'sprint': '047b', 'q': q, 'n': n, 'chi_max': chi,
                   'data': results_all, 'total_time': time.time()-t_start}, f, indent=2)

t_total = time.time() - t_start
print(f"\nTotal: {t_total:.1f}s")
print("Results saved to results/sprint_047b_q7_n12.json")
