"""Sprint 112b: Refit alpha(q) with q=12 data point.

7 data points (q=5-12). Tests linear, quadratic, power-law, logarithmic.
Uses last-pair pairwise alpha as best estimate (converging upward at all q).
"""
import numpy as np
import json, time, os
...
results = {
    'experiment': '112b_alpha_refit_q12',
    'sprint': 112,
...
def save():
    outpath = os.path.join(os.path.dirname(__file__), 'results', 'sprint_112b_alpha_refit_q12.json')
    os.makedirs(os.path.dirname(outpath), exist_ok=True)
    with open(outpath, 'w') as f:
        json.dump(results, f, indent=2, default=str)

# Collected alpha data: q, alpha_global, alpha_last_pair
# From sprints 103, 110, 111, 112a
data = {
...
print("=" * 70)
print("Sprint 112b: alpha(q) refit with q=12")
print("=" * 70)
...
for q_val in qs:
    ...
    print(f"  q={int(q_val):2d}: alpha_global={d['alpha_global']:.3f}, "
          f"alpha_last={d['alpha_last_pair']:.3f}")

# Use last-pair as primary (more accurate, less finite-size bias)
alphas = alphas_last
label = "last-pair"
...
n_data = len(qs)
print(f"\nModel fits ({label} alpha, {n_data} points):")
print("-" * 70)
...
for name, (func, n_params, param_names) in models.items():
    try:
        ...
        # Predictions at q=15, 20
        q15 = func(15.0, *popt)
        q20 = func(20.0, *popt)
        print(f"    -> q=15: {q15:.2f}, q=20: {q20:.2f}")
...
# Component analysis
print(f"\nComponent z_m(q) and beta_me(q) fits:")
print("-" * 70)
...
try:
    pb_pow, _ = curve_fit(power_law, qs, beta_mes)
...
recon_rms = np.sqrt(np.mean((alphas - alpha_recon)**2))
