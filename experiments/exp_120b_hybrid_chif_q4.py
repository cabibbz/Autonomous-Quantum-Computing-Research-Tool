"""Sprint 120b: chi_F spectral decomposition on HYBRID Potts-clock at q=4.

Hybrid q=4: g_c = 0.3933 (from Sprint 120a gap crossing, spread 0.0002).
q=4 is the marginal Ashkin-Teller point for S_q Potts (alpha=1.771).
Key question: does hybrid q=4 match S_q (alpha~1.77) or follow hybrid trend (alpha~1.4)?

Size limits: 4^n. n=4(256), 5(1024), 6(4096), 7(16k), 8(65k), 9(262k), 10(1M), 11(4.2M).
All feasible with GPU. Target: n=4-11 (8 data points).

Also runs q=3 (n=4-12) as sanity check: hybrid=S_q at q=3, alpha should be ~1.40.
"""
import numpy as np
import json, time, os
...
results = {
    'experiment': '120b_hybrid_chif_q4',
    'sprint': 120,
...
def save():
    outpath = os.path.join(os.path.dirname(__file__), '..', 'results', 'sprint_120b_hybrid_chif_q4.json')
...
def build_hybrid_parts_fast(n, q):
    """Build hybrid Potts-clock Hamiltonian parts: Potts delta coupling + (X+X†) field."""
    dim = q**n
...
def run_chif_spectral(q, g_c, sizes, k_need=12, label=''):
    ...
    for n in sizes:
        ...
        save()
        record(sprint=120, model='hybrid', q=q, n=n,
...
# Run experiments
# q=3 sanity check (hybrid = S_q at q=3): only a few sizes to be quick
run_chif_spectral(3, 0.333, [4, 6, 8, 10], k_need=10, label='(sanity: hybrid=S_q)')

# q=4 main target — 8 data points!
run_chif_spectral(4, 0.3933, [4, 5, 6, 7, 8, 9, 10, 11], k_need=12)
...
# Summary comparison
print(f"\n{'=' * 70}")
print("MODEL COMPARISON: HYBRID q=4 vs S_q q=4")
print("=" * 70)
sq_alpha = {3: 1.40, 4: 1.771, 5: 2.09, 7: 2.65}
hybrid_prior = {3: 1.40, 5: 1.408, 7: 0.952, 10: 0.552}
for q_val in [3, 4]:
    key = f'q{q_val}_global_alpha'
    if key in results:
        hybrid_a = results[key]
        sq_a = sq_alpha.get(q_val, None)
        sq_str = f"{sq_a:.3f}" if sq_a else "N/A"
        print(f"  q={q_val}: hybrid alpha={hybrid_a:.4f}, S_q alpha={sq_str}")

# Interpolation check
print(f"\n  --- Full hybrid alpha(q) trend ---")
for q_val, a_val in sorted(hybrid_prior.items()):
    print(f"  q={q_val:2d}: alpha={a_val:.3f} (prior)")
key4 = 'q4_global_alpha'
if key4 in results:
    print(f"  q= 4: alpha={results[key4]:.3f} (THIS SPRINT)")
