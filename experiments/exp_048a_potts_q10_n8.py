#!/usr/bin/env python3
"""Sprint 048a: q=10 Potts MI-CV sweep at n=8, chi=20.

Direct MPS contraction for all-pairs MI. Dense sweep around g_c≈0.246.
Rerun of Sprint 043 data with proper chi=20 (chi=10 was insufficient).
"""
import numpy as np
...
# === Timing test on single point ===
q = 10; n = 8; chi = 20
print(f"=== Timing test: q={q}, n={n}, chi={chi}, g=0.25 ===")
t0 = time.time()
cv, E0, mi_vals, t_dmrg, t_mi = mi_cv(n, q, 0.25, chi_max=chi)
dt = time.time() - t0
print(f"  CV={cv:.4f}, E0={E0:.6f}, DMRG={t_dmrg:.1f}s, MI={t_mi:.1f}s, total={dt:.1f}s")

if dt > 10:
    print(f"  WARNING: {dt:.0f}s per point, limiting sweep to key g values")
    g_values = [0.15, 0.20, 0.22, 0.24, 0.25, 0.26, 0.28, 0.30, 0.35]
else:
    g_values = [0.10, 0.15, 0.18, 0.20, 0.22, 0.24, 0.25, 0.26, 0.27, 0.28, 0.30, 0.35, 0.40]

# Already have g=0.25 result
results_all = [{
    'g': 0.25, 'cv': cv, 'E0': float(E0), 'mi_mean': float(np.mean(mi_vals)),
    'mi_vals': mi_vals, 'time_dmrg': t_dmrg, 'time_mi': t_mi, 'status': 'ok'
}]

print(f"\n=== q={q} Potts n={n} chi={chi} dense sweep (direct MPS) ===")
t_start = time.time()
...
for g in g_values:
    ...
    try:
        ...
        dt = time.time() - t0
        print(f"  g={g:.3f}: CV={cv:.4f}, E0={E0:.6f}, DMRG={t_dmrg:.1f}s, MI={t_mi:.1f}s, total={dt:.1f}s")
        results_all.append({
            'g': g, 'cv': cv, 'E0': float(E0), 'mi_mean': float(np.mean(mi_vals)),
            'mi_vals': mi_vals, 'time_dmrg': t_dmrg, 'time_mi': t_mi, 'status': 'ok'
        })
    except Exception as e:
        dt = time.time() - t0
        print(f"  g={g:.3f}: FAILED ({e.__class__.__name__}: {str(e)[:80]}), time={dt:.1f}s")
        results_all.append({'g': g, 'status': 'failed', 'error': str(e)[:200]})

    # Save after each point
    results_all_sorted = sorted([r for r in results_all], key=lambda x: x['g'])
    with open('results/sprint_048a_q10_n8.json', 'w') as f:
        json.dump({'sprint': '048a', 'q': q, 'n': n, 'chi_max': chi,
                   'data': results_all_sorted, 'total_time': time.time()-t_start}, f, indent=2)

t_total = time.time() - t_start
print(f"\nTotal: {t_total:.1f}s")
print("Results saved to results/sprint_048a_q10_n8.json")
