"""
Sprint 042d: True q=5 Potts n=12 — crossing test.
Key question: Does CV increase or decrease with n?
If first-order: CV increases at ALL g above transition (no crossing).
If second-order: CV decreases in ordered phase but increases in disordered (crossing).

n=8 reference: g=0.30 CV=0.157, g=0.40 CV=0.547, g=0.50 CV=1.065
"""
...
def mi_cv_potts(n, g_J, chi_max=20):
    ...
    eng = dmrg.TwoSiteDMRGEngine(psi, model, {
        'mixer':True,'max_E_err':1e-6,'trunc_params':{'chi_max':chi_max},'max_sweeps':8})
    E0,_ = eng.run()
...
print("=== Sprint 042d: q=5 Potts n=12 — Crossing Test ===\n")
sys.stdout.flush()

results = {'experiment':'042d','model':'q=5 Potts','n':12,'chi_max':20,'data':{}}

# Strategic points: ordered (0.30), transition (0.40), disordered (0.50)
for g_J in [0.30, 0.40, 0.50]:
    elapsed=time.time()-t0
    if elapsed>145:
        print(f"  TIMEOUT {elapsed:.0f}s"); break
    t1=time.time()
    try:
        cv=mi_cv_potts(12, g_J, chi_max=20)
        dt=time.time()-t1
    ...
    with open('results/sprint_042d_potts_q5_n12.json','w') as f:
        json.dump(results, f, indent=2)

print(f"\nTotal: {time.time()-t0:.1f}s")
print("\n=== Crossing Analysis ===")
n8_ref = {0.30: 0.1567, 0.40: 0.5466, 0.50: 1.0645}
for g_str, d in results['data'].items():
    if d.get('cv') is not None:
        ...
        print(f"  g={g:.2f}: n=8 CV={n8_cv:.4f} -> n=12 CV={n12_cv:.4f} ({direction} by {delta:+.4f})")

print("\nFirst-order prediction: ALL points increase (no crossing)")
print("Second-order prediction: Ordered decreases, disordered increases (crossing)")
