"""
Sprint 039b2: Potts n=24 MI-CV — 2 key points for crossing detection.
g=0.8 (ordered) and g=1.0 (transition). chi_max=40.
"""
...
def mi_cv_potts(n, g_J, chi_max=40):
...
print("=== Sprint 039b2: Potts n=24 sparse ===\n")
sys.stdout.flush()

results = {'experiment': '039b2', 'model': 'q=3 Potts (clock)', 'n': 24, 'data': {}}

for g_J in [0.8, 0.95, 1.0]:
    elapsed = time.time() - t0
    if elapsed > 48:
        print(f"  TIMEOUT at {elapsed:.0f}s")
        break
    t1 = time.time()
    cv = mi_cv_potts(24, g_J, chi_max=40)
    dt = time.time() - t1
    print(f"  g/J={g_J:.2f}: CV={cv:.4f} ({dt:.1f}s)")
    sys.stdout.flush()
    results['data'][str(g_J)] = {'cv': float(cv), 'time': float(dt)}
    results['total_runtime'] = time.time() - t0
    with open('results/sprint_039b2_potts_n24.json', 'w') as f:
