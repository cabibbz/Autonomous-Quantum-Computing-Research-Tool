"""
Sprint 038b2: Complete Potts MI-CV — n=8 full sweep + n=12 sparse.
Uses conserve=None (required for Gell-Mann operators) with low chi.
"""
...
def mi_cv_potts(n, g_J, chi_max=25):
...
print("=== Sprint 038b2: Potts MI-CV (fast) ===\n")
sys.stdout.flush()

results = {'experiment': '038b2', 'model': 'q=3 Potts (clock)', 'sizes': {}}

# Load prior n=8 data (from 038b)
with open('results/sprint_038b_potts_micv.json') as f:
    prior = json.load(f)
results['sizes']['8'] = prior['sizes']['8']

# Complete n=8: missing g=1.1, 1.2, 1.3, 1.5, 2.0
missing_8 = [g for g in [1.1, 1.2, 1.3, 1.5, 2.0]
             if str(g) not in results['sizes']['8']]
print(f"--- Completing n=8 ({len(missing_8)} missing points) ---")
sys.stdout.flush()
for g_J in missing_8:
    if time.time() - t0 > 25:
        print(f"  TIMEOUT"); break
    t1 = time.time()
    cv = mi_cv_potts(8, g_J, chi_max=25)
    dt = time.time() - t1
    print(f"  g/J={g_J:.2f}: CV={cv:.4f} ({dt:.1f}s)")
    sys.stdout.flush()
    results['sizes']['8'][str(g_J)] = {'cv': float(cv), 'time': float(dt)}

# Save after n=8
results['total_runtime'] = time.time() - t0
with open('results/sprint_038b2_potts_fast.json', 'w') as f:
    json.dump(results, f, indent=2)

# n=12 sparse
print("\n--- n=12 (chi=30) ---")
sys.stdout.flush()
g_vals_12 = [0.5, 0.8, 0.95, 1.0, 1.05, 1.2]
results['sizes']['12'] = {}
for g_J in g_vals_12:
    if time.time() - t0 > 52:
        print(f"  TIMEOUT"); break
    t1 = time.time()
    cv = mi_cv_potts(12, g_J, chi_max=30)
    dt = time.time() - t1
    print(f"  g/J={g_J:.2f}: CV={cv:.4f} ({dt:.1f}s)")
    sys.stdout.flush()
    results['sizes']['12'][str(g_J)] = {'cv': float(cv), 'time': float(dt)}
