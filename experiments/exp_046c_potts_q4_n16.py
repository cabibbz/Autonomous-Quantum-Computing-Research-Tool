#!/usr/bin/env python3
"""Sprint 046c: q=4 Potts MI-CV sweep at n=16, chi=20.

Direct MPS contraction. Same g range as n=8,12 plus lower extension.
"""
import numpy as np
...
n = 16
chi = 20
# Extended range: go lower to find crossing, plus original range
g_values = [0.40, 0.50, 0.60, 0.70, 0.80, 0.85, 0.90, 0.95, 1.00, 1.05]
...
for g in g_values:
    ...
    with open('results/sprint_046c_q4_n16.json', 'w') as f:
        json.dump({'sprint': '046c', 'q': q, 'n': n, 'chi_max': chi,
                   'data': results_all, 'total_time': time.time()-t_start}, f, indent=2)

t_total = time.time() - t_start
print(f"\nTotal: {t_total:.1f}s")
print("Results saved to results/sprint_046c_q4_n16.json")
