#!/usr/bin/env python3
"""Sprint 048b: q=10 Potts MI-CV sweep at n=12, chi=20.

Direct MPS contraction for all-pairs MI. Focused sweep around g_c≈0.246.
n=12 is ~3x slower than n=8 (66 pairs vs 28). Use fewer g values.
"""
import numpy as np
...
q = 10; n = 12; chi = 20
# Focused sweep: key points for crossing detection and slope extraction
g_values = [0.15, 0.20, 0.24, 0.25, 0.26, 0.28, 0.30, 0.35]

results_all = []
print(f"=== q={q} Potts n={n} chi={chi} sweep (direct MPS) ===")
t_start = time.time()
...
for g in g_values:
    ...
    with open('results/sprint_048b_q10_n12.json', 'w') as f:
        json.dump({'sprint': '048b', 'q': q, 'n': n, 'chi_max': chi,
                   'data': results_all, 'total_time': time.time()-t_start}, f, indent=2)

t_total = time.time() - t_start
print(f"\nTotal: {t_total:.1f}s")
print("Results saved to results/sprint_048b_q10_n12.json")
