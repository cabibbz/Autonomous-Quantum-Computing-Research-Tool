#!/usr/bin/env python3
"""Sprint 044b: q=7 Potts MI-CV sweep at n=8.
Predictions from 044a: g_c(7) in [0.24, 0.49], mean 0.32.
Sweep g = 0.15 to 0.55 in steps of 0.05 to map the transition.
"""
import numpy as np
...
q = 7
n = 8
g_values = [0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 0.55]

results_all = []
print(f"=== q={q} Potts n={n} MI-CV sweep ===")
t_start = time.time()
...
for g in g_values:
    ...
    with open('results/sprint_044b_q7_n8_sweep.json', 'w') as f:
        json.dump({'sprint': '044b', 'q': q, 'n': n, 'chi_max': 10,
...
if len(ok) >= 2:
    ...
    for i in range(len(ok)-1):
        dg = ok[i+1]['g'] - ok[i]['g']
        dcv = ok[i+1]['cv'] - ok[i]['cv']
        slopes.append((ok[i]['g'] + dg/2, dcv/dg))
    max_slope = max(slopes, key=lambda x: abs(x[1]))
    print(f"\nSteepest slope at g≈{max_slope[0]:.2f}: dCV/dg={max_slope[1]:.2f}")
