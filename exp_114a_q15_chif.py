"""Sprint 114a: chi_F spectral decomposition for q=15 at n=3,4,5.

Discriminates quadratic vs sqrt(q) models for alpha(q):
  Quadratic (-0.010q^2+0.41q+0.30): alpha = 4.20
  sqrt(q)   (1.35*sqrt(q)-0.91):    alpha = 4.32
  Difference: 0.12 (3%)

Dimensions: n=3 (3.4k), n=4 (51k), n=5 (759k GPU).
"""
import numpy as np
import json, time, os
...
def save():
    outpath = os.path.join(os.path.dirname(__file__), 'results', 'sprint_114a_q15_chif.json')
...
g_c = 1.0 / q
all_sizes = [3, 4, 5]
k_need = 18  # (q-1)=14-fold multiplet + singlet + margin

print("=" * 70)
print(f"Sprint 114a: q={q} chi_F spectral decomposition, n={all_sizes}")
print(f"  g_c = {g_c:.6f}, k = {k_need}")
print(f"  Quadratic prediction: alpha = {-0.010*q**2 + 0.41*q + 0.30:.3f}")
print(f"  sqrt(q) prediction:   alpha = {1.35*q**0.5 - 0.91:.3f}")
print("=" * 70)
...
for n in all_sizes:
    ...
    record(sprint=114, model='sq_potts', q=q, n=n,
...
if len(Ns) >= 2:
    ...
    alpha_quad = -0.010 * q**2 + 0.41 * q + 0.30
    alpha_sqrt = 1.35 * q**0.5 - 0.91
    alpha_linear = 0.260 * q + 0.827
    alpha_log = 2.50 * np.log(q) - 2.60  # log model from Sprint 112
    print(f"\n  Predictions:")
    print(f"    Quadratic (-0.010q^2+0.41q+0.30): {alpha_quad:.3f}")
    print(f"    sqrt(q)   (1.35*sqrt(q)-0.91):    {alpha_sqrt:.3f}")
    print(f"    Log       (2.50*ln(q)-2.60):      {alpha_log:.3f}")
    print(f"    Linear    (0.260q+0.827):          {alpha_linear:.3f}")
    print(f"    Measured (global):                 {alpha_global:.3f}")
    for name, pred in [('Quadratic', alpha_quad), ('sqrt(q)', alpha_sqrt),
                       ('Log', alpha_log), ('Linear', alpha_linear)]:
