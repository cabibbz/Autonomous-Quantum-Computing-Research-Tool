"""Sprint 110a: Extend q=6 chi_F spectral decomposition to 4 sizes (n=6,7,8,9).

Currently q=6 has only 2 sizes (n=6,8) from Sprint 103b — the weakest link in
the alpha(q) formula. Adding n=7 (dim=279k) and n=9 (dim=10M, GPU) gives 4 sizes.

Uses vectorized Hamiltonian builder (Sprint 098) for fast construction.
"""
import numpy as np
import json, time, os
...
def save():
    outpath = os.path.join(os.path.dirname(__file__), 'results', 'sprint_110a_q6_extend.json')
...
g_c = 1.0 / q
all_sizes = [6, 7, 8, 9]
k_need = 8  # q=6: (q-1)=5-fold multiplet + singlet above it + margin

print("=" * 70)
print(f"Sprint 110a: q={q} chi_F spectral decomposition, n={all_sizes}")
...
alpha_global = p_a[0]
print(f"\n  Global alpha (all 4 sizes): {alpha_global:.4f}")
