"""Sprint 115a: chi_F spectral decomposition for q=20 at n=3,4,5.

Decisive test: logarithmic predicts alpha=4.49, quadratic predicts 3.83.
Difference 0.66 (15%) — easily distinguishable.

Dimensions: n=3 (8k), n=4 (160k), n=5 (3.2M GPU).
"""
import numpy as np
import json, time, os
...
def save():
    outpath = os.path.join(os.path.dirname(__file__), 'results', 'sprint_115a_q20_chif.json')
...
g_c = 1.0 / q
all_sizes = [3, 4, 5]
k_need = 24  # (q-1)=19-fold multiplet + singlet + margin

print("=" * 70)
print(f"Sprint 115a: q={q} chi_F spectral decomposition, n={all_sizes}")
print(f"  g_c = {g_c:.6f}, k = {k_need}")
print(f"  Log prediction:  alpha = {1.726*np.log(q) - 0.684:.3f}")
print(f"  Quad prediction: alpha = {-0.0134*q**2 + 0.451*q + 0.159:.3f}")
print("=" * 70)
...
for n in all_sizes:
    ...
    record(sprint=115, model='sq_potts', q=q, n=n,
...
if len(Ns) >= 2:
    ...
    alpha_log = 1.726 * np.log(q) - 0.684
    alpha_quad = -0.0134 * q**2 + 0.451 * q + 0.159
    alpha_sqrt = 1.150 * q**0.5 - 0.392
    alpha_power = 0.900 * q**0.556
    alpha_linear = 0.185 * q + 1.343
    print(f"\n  Predictions (from Sprint 114b fits):")
    print(f"    Logarithmic (1.726*ln(q)-0.684):       {alpha_log:.3f}")
    print(f"    Quadratic (-0.0134q^2+0.451q+0.159):   {alpha_quad:.3f}")
    print(f"    sqrt(q) (1.150*sqrt(q)-0.392):         {alpha_sqrt:.3f}")
    print(f"    Power-law (0.900*q^0.556):              {alpha_power:.3f}")
    print(f"    Linear (0.185q+1.343):                  {alpha_linear:.3f}")
    print(f"    Measured (global):                      {alpha_global:.3f}")
    for name, pred in [('Logarithmic', alpha_log), ('Quadratic', alpha_quad),
                       ('sqrt(q)', alpha_sqrt), ('Power-law', alpha_power),
                       ('Linear', alpha_linear)]:
    ...
    results['pairwise'] = {
        ...
        'alpha_pred_sqrt': float(alpha_sqrt),
