#!/usr/bin/env python3
"""Sprint 047a: q=7 Potts MI-CV sweep at n=8, chi=20.

Direct MPS contraction for all-pairs MI. Dense sweep around g_c≈0.259.
Timing test for d=7 at n=8.
"""
import numpy as np
...
n = 8
chi = 20
# Dense sweep around g_c=0.259, plus broader context
g_values = [0.15, 0.20, 0.22, 0.24, 0.25, 0.26, 0.27, 0.28, 0.30, 0.35, 0.40]

results_all = []
print(f"=== q={q} Potts n={n} chi={chi} dense sweep (direct MPS) ===")
t_start = time.time()
...
for g in g_values:
    ...
    with open('results/sprint_047a_q7_n8.json', 'w') as f:
        json.dump({'sprint': '047a', 'q': q, 'n': n, 'chi_max': chi,
                   'data': results_all, 'total_time': time.time()-t_start}, f, indent=2)

t_total = time.time() - t_start
print(f"\nTotal: {t_total:.1f}s")
print("Results saved to results/sprint_047a_q7_n8.json")
