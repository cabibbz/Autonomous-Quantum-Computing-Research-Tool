"""Sprint 111a: chi_F spectral decomposition for q=10 at n=5,6,7.

Extends alpha(q) to q=10. Predictions:
  Linear (0.262q+0.815):    alpha = 3.44
  Quadratic (-0.009q²+0.389q+0.386): alpha = 3.28

Dimensions: n=5 (100k), n=6 (1M), n=7 (10M GPU).
Uses vectorized S_q Potts builder.
"""
import numpy as np
import json, time, os
...
def save():
    outpath = os.path.join(os.path.dirname(__file__), 'results', 'sprint_111a_q10_chif.json')
...
g_c = 1.0 / q
all_sizes = [5, 6, 7]
k_need = 12  # (q-1)=9-fold multiplet + singlet + margin

print("=" * 70)
print(f"Sprint 111a: q={q} chi_F spectral decomposition, n={all_sizes}")
...
for n in all_sizes:
    ...
    record(sprint=111, model='sq_potts', q=q, n=n,
...
if len(Ns) >= 2:
    ...
    alpha_linear = 0.262 * q + 0.815
    alpha_quad = -0.009 * q**2 + 0.389 * q + 0.386
    print(f"\n  Predictions:")
    print(f"    Linear  (0.262q+0.815):  {alpha_linear:.3f}")
